...
def test_signup_and_check(new_user_with_delete):
    ...
    with allure.step('Проверяем результат фикстуры'):
        ...
        if not new_user_with_delete.get('success', False):
            ...
            return
        #print (new_user_with_delete['access_token'])

    with allure.step('Получаем данные профиля из бд'): # Если успешно - продолжается работа теста
    ...
    with allure.step('Сверяем данные профиля'):
...
def test_user_delete_and_verify():
    ...
    with allure.step('Delete profile'): # Удаляем профиль
        headers = {'Authorization': f"Bearer {access_token}"}
    ...
    with allure.step('Check the profile is unavailable'): # Проверяем что профиль недоступен
        profile_response = requests.get(
            f'{BASE_URL}/profile',
            headers=headers,
            timeout=10
